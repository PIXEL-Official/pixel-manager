...
import { VoiceChannel } from '../models/types';

export class VoiceChannelRepository {
  /**
   * 새로운 음성 채널 추가
   */
  async addChannel(channel: Omit<VoiceChannel, 'id' | 'created_at' | 'updated_at'>): Promise<VoiceChannel | null> {
    // 중복 체크
   ...
   * 채널 ID로 조회
   */
  async getChannelById(channelId: string, guildId: string): Promise<VoiceChannel | null> {
    const { data, error } = await supabase
      .from('voice_channels')
      .select('*')
   ...
   * 길드의 모든 활성 채널 조회
   */
  async getActiveChannels(guildId: string): Promise<VoiceChannel[]> {
    const { data, error } = await supabase
      .from('voice_channels')
      .select('*')
      .eq('guild_id', guildId)
   ...
   * 길드의 모든 채널 조회 (비활성 포함)
   */
  async getAllChannels(guildId: string): Promise<VoiceChannel[]> {
    const { data, error } = await supabase
      .from('voice_channels')
      .select('*')
      .eq('guild_id', guildId)
...
export const voiceChannelRepository = new VoiceChannelRepository();
